...
const Simulador = () => {
  ...
  const handleSubmit = (e) => {
    ...
    if (!objetivoValido) {
      alert(
        "⚠️ Debes ingresar al menos un coeficiente distinto de cero en la función objetivo."
      );
      return;
    }
    ...
    const restriccionInvalida = formData.restricciones.some((fila, index) => {
      ...
      const resultadoValido =
        formData.resultados[index] !== "" &&
        !isNaN(parseFloat(formData.resultados[index]));
      return !coeficientesValidos || !resultadoValido;
    });

    if (restriccionInvalida) {
      alert(
        "⚠️ Cada restricción debe tener al menos un coeficiente distinto de cero y un resultado numérico."
      );
      return;
    }
                  ...
                  <tr key={restriccionIndex}>
                    ...
                    <td>
                      <input
                        ...
                        onChange={(e) =>
                          handleResultChange(e, restriccionIndex)
                        }
                        className="p-2 w-20 border border-gray-300 rounded-lg"
                      />
                    </td>
